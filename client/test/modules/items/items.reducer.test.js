...
import { addBucket, addBucketGroup, updateItem, deleteItem, addTransactionToItem } from '../../../src/modules/items/items.actions.js';

const testItems = {
    'group': { label: 'Something', items: ['item__in-group', 'item__in-group__2'] },
    'item__in-group': { label: null, value: { type: 'static', amount: 3 }},
    'item__in-group__2': { label: 'in group2!', value: { type: 'static', amount: 3 }},
...
describe('items reducer', () => {
    ...
    it('should add a root bucket', () => {
        ...
        let newBucketKey = Object.keys(items).filter(key => !Object.keys(testItems).includes(key))[0]
    ...
    it('should add bucket group', () => {
        ...
        let newBucketGroupKey = Object.keys(items).filter(key => !Object.keys(testItems).includes(key))[0]
    ...
    it('should update item', () => {
        let updatedItem = { label: 'heyy', value: {type: 'income', amount: 99} };
    ...
    it('should delete item not in group', () => {
        ...
        let expectedItemKeys = Object.keys(testItems).filter(key => key != deleteKey)
    ...
    it('should delete item in group', () => {
        ...
        let expectedItemKeys = Object.keys(testItems).filter(key => key != deleteKey)
        let expectedGroupKeys = testItems['group'].items.filter(key => key != deleteKey);
